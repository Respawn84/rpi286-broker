...
SCREEN_WIDTH = 70
# Ancho de los marcos ASCII de los menus. Estuvo en 62 mientras cada
# caracter costaba CHAR_DELAY en el cable y un marco mas estrecho se
# pintaba antes. Con el freno quitado y a 28800 esos ocho caracteres de
# mas no se notan, y se agradecen en los titulares de prensa, que van
# justos de sitio en la linea del menu.
BOX_WIDTH = 70
...
#
# Se aplica SOLO al chat libre (opcion 5), que es el unico sitio donde
...
# El indice de canales de Europa Press, en formato OPML: un fichero que
# lista todos sus feeds ya agrupados por temas. De ahi salen los menus
# del 286, asi que si el medio anade una seccion aparece sola sin tocar
# codigo. Para cambiar de periodico basta con otro OPML aqui, siempre
# que sea RSS estandar.
NOTICIAS_OPML = "https://www.europapress.es/rss/europapress.opml.xml"
# En api.env por si se quiere otro medio en una maquina concreta.
NOTICIAS_OPML = leer_env("NOTICIAS_OPML", NOTICIAS_OPML)

NOTICIAS_TIMEOUT = 15
# El indice de canales cambia como mucho cada meses: se pide una vez y
# vale para toda la sesion. Los feeds si se mueven, asi que su cache es
# corta; lo justo para que volver atras en el menu y entrar otra vez no
# cueste otra descarga.
NOTICIAS_OPML_CACHE = 86400
NOTICIAS_CACHE = 300
# Tope de titulares por canal. Los feeds traen entre 25 y 50; mas de
# esto es una lista por la que ya no apetece navegar desde el 286.
NOTICIAS_MAX_ARTICULOS = 30

# Opciones que caben en un menu antes de partirlo en paginas. La zona
# de contenido de CHAT.EXE son 18 filas y el marco se lleva cuatro (o
# seis si hay pie), asi que 12 entra siempre con sitio de sobra. Sin
# esto, el grupo "Actualidad Autonomica" (17 canales) se saldria por
# arriba y la mitad de la lista ya no estaria en pantalla al elegir.
MENU_MAX_OPCIONES = 12
